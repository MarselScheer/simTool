...
test_that("Default function for analyzing the generated data is the function length",
          expect_identical(eg$procGrid, expandGrid(proc="length")))
...
test_that("The cluster libraries are loaded on the cluster", 
          expect_true(all(as.data.frame(eg)$ret)))
...
test_that("Error if deprecated parameter post.proc is used",
          expect_error(evalGrids(dg, pg, post.proc=mean)))
...
test_that("summary.fun from as.data.frame and evalGrids return the same object",
...
test_that("summary.fun from as.data.frame works with a function returning a vector",
...
df2 = as.data.frame(eg)
df2$replication=NULL
test_that("summary.fun from evalGrids works with a function returning a vector",
          expect_identical(df3, df2))
