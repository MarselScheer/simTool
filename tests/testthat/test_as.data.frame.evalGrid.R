...
test_that("Results are stored in the correct place",
          expect_identical(df$V1, tmp))

test_that("Deprecated parameters value.fun, post.proc from as.data.frame generate an error",{
  expect_error(as.data.frame(eg, value.fun=identity))
  expect_error(as.data.frame(eg, post.proc=mean))
})
...
test_that("summary.fun can handel logical data", 
          expect_identical(as.data.frame(eg, convert.result.fun=f, summary.fun=c(mean, median)), result))

postVec = function(results)  summary(results)

result = structure(list(i = 1:4, j = c(1L, 1L, 1L, 1L), fun = c("runif", "runif", "runif", "runif"), 
               n = 1:4, proc = c("mean", "mean", "mean", "mean"), 
               value = structure(c(1L, 1L, 1L, 1L), .Label = "(all)", class = "factor"), 
...
test_that("summary.fun can handel logical data and returning vector", 
          expect_identical(as.data.frame(eg, convert.result.fun=f, summary.fun=postVec), result))
...
test_that("Variable names are introduced, where the analyzing function does not provide names", 
...
test_that("Check that results can be converted to a data.frame", 
...
tmp = rbind(tmp1, tmp2)
test_that("Check that results can be converted to a data.frame and summarized over the replications", 
...
tmp = rbind(tmp1, tmp2)
test_that("Check that results can be converted to a data.frame and summarized (vector) over the replications", 
          expect_equivalent(df[,-(1:5)], tmp))
...
test_that("Create an error to check afterwards that the results so far were stored",
          expect_error(eg <- evalGrids(dg, pg, replications=4, progress=TRUE, fallback="RUnitFallBack", envir=environment())))
rm(list=ls())
...
test_that("Fallback object was correctly stored to hard drive", 
