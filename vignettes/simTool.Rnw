...
\texttt{evalGrids} which conducts the simulation. Afterwards, the results can nicely
be displayed as a \texttt{data.frame} be coercing the object returned by \texttt{evalGrids}
to a \texttt{data.frame}.

\section{Defining the \texttt{data.frames} for data generation and analyzation}
...
minimum and the arithmetic mean with \texttt{trim=0.1} and \texttt{trim=0.2}.
...
\texttt{evalGrids} saves ALL generated data sets.
...
\texttt{discardGeneratedData = TRUE}.
...
this does not make sense if one uses \texttt{Sweave} or \texttt{knitr},
...
<<>>=
require(parallel)
cl = makeCluster(rep("localhost", 3), type="PSOCK") 
eg = evalGrids(dataGrid = dg, procGrid = pg, replications = 10, 
...
The function \texttt{ratio} exists at the moment only in our global environment.
...
Of course, it is possible to set \texttt{clusterGlobalObjects=ls()}, but then 
...
of the class \texttt{evalGrid}. Of course, some results are missing 
which is indicated by the column \linebreak \texttt{.evalGridComment} in the 
resulting \texttt{data.frame}.
...
and \texttt{procGrid}. This is achieved by applying
the \texttt{R}-function
\texttt{get}. By default, \texttt{envir=globalenv()}
and thus \texttt{get} 
...
\texttt{as.data.frame} can coerce it to a \texttt{data.frame}. Nevertheless,
we want to discuss the return value of \texttt{evalGrids}.
<<>>=
dg = rbind.fill(
  expandGrid(fun="rexp", n=c(10, 20), rate=1:2),
  expandGrid(fun="rnorm", n=c(10, 20), mean=1:2))
...
@
The returned object is a \texttt{list} of class \texttt{evalGrid}:
<<>>=
names(eg)
@
The important element is \texttt{simulation}
...
\texttt{eg\$simulation[[i]][[r]]\$data} is the data generated by the $i$th row in \texttt{dg}
in the $r$th replication and \texttt{eg\$simulation[[i]][[r]]\$results[[j]]} is the object
returned by the $j$th parameter constellation of \texttt{pg} applied to \linebreak
...
\section{Converting results to \texttt{data.frame}}

We have already applied \texttt{as.data.frame.evalGrid} many times. This function
also has the parameters \texttt{post.proc} and \texttt{progress}. The functionality
of these parameter resembles the corresponding parameters of \texttt{evalGrids}.
Hence, it remains to explain \texttt{value.fun}. Sometimes, the objects returned by
the analyzing functions can not be automatically converted to \texttt{data.frame}.
In such cases, the parameter \texttt{value.fun} enables the user to pre-process
...
@ 
An object of class \texttt{lm} can easily be converted by
calling \texttt{coef}.
<<>>=
head(df<-as.data.frame(eg, value.fun=coef))
@
Of course, this can be combined with \texttt{post.proc}
<<>>=
