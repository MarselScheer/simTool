...
as.data.frame(eg)
```
Note, by specifying the parameter *summary.fun* the generated data sets
...
*as.data.frame.evalGrid* has also a parameter *summary.fun* that
serves the same purpose, it may be necessary to summarize the results as soon
as possible to spare memory. 

We now briefly show that *summary.fun* in *evalGrids* and
...
also has the parameters *summary.fun* and *progress*. The functionality
of these parameter resembles the corresponding parameters of *evalGrids*.
Hence, it remains to explain *convert.result.fun*. Sometimes, the objects returned by
the analyzing functions can not be automatically converted to *data.frame*.
In such cases, the parameter *convert.result.fun* enables the user to pre-process
...
```
Of course, this can be combined with *summary.fun*
```{r}
