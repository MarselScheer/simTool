...
See command line 11 in the pseudo code.
```{r}
eg = eval_tibbles(data_grid = dg, proc_grid = pg, replications = 1000)
object.size(eg)
eg = eval_tibbles(data_grid = dg, proc_grid = pg, replications = 1000,
    discard_generated_data = TRUE)
object.size(eg)
```
The object returned by *eval_tibbles* will be discussed at the
...
eval_tibbles(data_grid = dg, proc_grid = pg, replications = 1000, 
...
next section.
```{r}
eval_tibbles(data_grid = dg, proc_grid = pg, replications = 10, 
...
eval_tibbles(dg, pg, replications=10, ncpus=2,
