...
Somtimes it is handy to access the parameter constellation that was used
during the data generation in the (post) data analyzing phase. If the
(post) data anaylzing function has an argument *.truth*, then 
*eval_tibbles* will manage that the hand-over. A brief example should
...
pg <- expand_tibble(proc = "quantile", probs = 0.975)
post_ana <- function(q_est, .truth){
  tibble::tibble(bias = q_est - .truth)
}
eval_tibbles(dg, pg, replications = 10^3, discard_generated_data = TRUE,
                   ncpus = 2,
                   post_analyze = post_ana,
...
In the same fashtion one could write a data analyzing function with a 
parameter *.truth*.
